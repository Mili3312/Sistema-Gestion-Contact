...
const ContactForm = ({ onContactAdded, onContactEdited, editingContact }) => {
  ...
  return (
    <div className='max-w-xl mx-auto p-4 bg-white rounded-lg shadow-md'>
      <h2 className='text-2xl font-bold text-teal-600 mb-4'>{editingContact ? 'Editar Contacto' : 'Nuevo contacto'}</h2>

      <form onSubmit={handleSubmit} className='space-y-4'>
        <div>
          <label htmlFor="name" className='block text-lg'>Nombre</label>
          <input type="text" id='name' name='name' value={name} onChange={(e) => setName(e.target.value)} className='w-full p-2 border rounded-lg text-gray-700 bg-teal-100' placeholder='nombre completo' />
          {errors.name && <p className='text-red-500'>{errors.name}</p>}
        </div>

        <div>
          <label htmlFor="email" className='block text-lg'> Correo electronico</label>
          <input type="email" name="email" id="email" value={email} onChange={(e) => setEmail(e.target.value)} className='w-full p-2 border rounded-lg text-gray-700 bg-teal-100' placeholder='correo electrónico' />
          {errors.email && <p className='text-red-500'>{errors.email}</p>}
        </div>

        <div>
          <label htmlFor="phone" className='block text-lg'> Teléfono </label>
          <input type="text" name="phone" id="phone" value={phone} onChange={(e) => setPhone(e.target.value)} className='w-full p-2 border rounded-lg text-gray-700 bg-teal-100' placeholder='Número de teléfono' />
          {errors.phone && <p className='text-red-500'>{errors.phone}</p>}
        </div>

        <div>
          <label htmlFor="address" className='block text-lg'> Dirección </label>
          <input type="text" name="address" id="address" value={address} onChange={(e) => setAddress(e.target.value)} className='w-full p-2 border rounded-lg text-gray-700 bg-teal-100' placeholder='Dirección' />
          {errors.address && <p className='text-red-500'>{errors.address}</p>}
        </div>

        <div>
          <label htmlFor="birthDate" className='block text-lg'> fecha de nacimiento</label>
          <input type="date" name="birthDate" id="birthDate" value={birthDate} onChange={(e) => setBirthDate(e.target.value)} className='w-full p-2 border rounded-lg text-gray-700 bg-teal-100' />
